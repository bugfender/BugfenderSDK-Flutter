...
class FlutterBugfender {
  ...
  ///
  /// If [logLevel] is provided it'll be used as LogLevel for the log.
  /// Otherwise [LogLevel.Info] will be applied.
  ///
  /// If [tag] is provided it'll be used as Tag for the log.
  /// Otherwise 'Info' tag will be applied.
  ///
  ...
  static Future<dynamic> l(
    ...
    final logLevelString = logLevel?.toString()?.split('.')?.elementAt(1);
  ...
  static Future<dynamic> warn(String value, {String tag}) {
    Map<String, String> map = <String, String>{
      'log': value,
      'tag': tag ?? 'Warning',
    };

    return _channel.invokeMethod('warn', map);
  }

  /// Error Bugfender log method.
  static Future<dynamic> error(String value, {String tag}) {
    Map<String, String> map = <String, String>{
      'log': value,
      'tag': tag ?? 'Error',
    };

    return _channel.invokeMethod('error', map);
  }
